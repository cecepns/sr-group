...
import { getPengeluaran, postPengeluaran, putPengeluaran, deletePengeluaran, getLocations } from '../services/api';
...
export default function PengeluaranKantor() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.nama_pengeluaran.trim() || !form.jumlah) return;
    setSubmitting(true);
    try {
      const payload = {
      ...
      if (editId) {
        await putPengeluaran(editId, payload);
      } else {
        await postPengeluaran(payload);
      }
      setModalOpen(false);
      await load(page);
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Pengeluaran Kantor</h1>
      ...
      <div className="bg-white rounded-2xl shadow border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h2 className="text-lg font-semibold text-slate-800">Daftar Pengeluaran Kantor</h2>
              ...
              <tbody className="divide-y divide-slate-200">
                {list.map((row, i) => (
                  <tr key={row.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">{row.nama_pengeluaran}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Pengeluaran Kantor' : 'Tambah Pengeluaran Kantor'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Nama Pengeluaran</label>
            <input
              type="text"
              value={form.nama_pengeluaran}
              onChange={(e) => setForm((f) => ({ ...f, nama_pengeluaran: e.target.value }))}
