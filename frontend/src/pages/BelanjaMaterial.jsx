...
import { getBelanjaMaterial, postBelanjaMaterial, getMaterials } from '../services/api';
...
export default function BelanjaMaterial() {
  ...
  const loadList = async (p = page) => {
    try {
      setLoading(true);
      const res = await getBelanjaMaterial({ page: p, limit });
      const body = res.data;
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Tambah Belanja Material" size="lg">
